...
```sql
SELECT * FROM invoices WHERE BillingState = 'NULL' ORDER BY InvoiceDate DESC LIMIT 5; 
```
...
```sql
SELECT COUNT(*) FROM invoices WHERE strftime = (SELECT strftime ('%Y','InvoiceDate') = 2013 from invoices); 
...
```sql
SELECT ArtistId, COUNT(*) FROM albums WHERE ArtistId = (SELECT ArtistId FROM artists); 
